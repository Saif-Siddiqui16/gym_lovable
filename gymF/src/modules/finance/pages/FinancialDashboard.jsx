            ...
            <div className="max-w-screen-2xl mx-auto space-y-10">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-8 min-w-0 bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100 flex flex-col">
                        ...
                        <div className="flex-1 min-h-[350px] overflow-x-auto scrollbar-hide pb-4">
                            <div className="flex items-end justify-between gap-4 md:gap-6 min-w-[600px] h-full px-2">
                                {monthlyData.map((m, idx) => {
                                    ...
                                    return (
                                        <div key={idx} className="flex-1 flex flex-col items-center gap-6 group">
                                            ...
                                            <span className="text-[11px] font-black text-slate-500 uppercase tracking-widest">{m.month}</span>
                                        </div>
                                    );
                                })}
                            </div>
                    ...
                    <div className="lg:col-span-4 min-w-0 bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100 flex flex-col">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-10">
                            <div className="bg-[#f0fdf4] p-5 rounded-3xl border border-emerald-100 text-center group cursor-pointer hover:shadow-lg hover:shadow-emerald-50 transition-all">
                                <h4 className="text-emerald-700 text-lg font-black truncate">₹{summary.netProfit.toLocaleString()}</h4>
                                <p className="text-[#15803d] text-[9px] font-black uppercase tracking-widest mt-2">Net Profit</p>
                            </div>
                            <div className="bg-[#fef2f2] p-5 rounded-3xl border border-red-100 text-center group cursor-pointer hover:shadow-lg hover:shadow-red-50 transition-all">
                                <h4 className="text-red-700 text-lg font-black truncate">₹{summary.totalExpenses.toLocaleString()}</h4>
                                    ...
                                    {transactions.slice(0, 5).map((txn, idx) => (
                                        <div key={idx} className="flex items-center justify-between p-4 bg-slate-50/50 rounded-2xl hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100 group">
                                            <div className="flex items-center gap-3 min-w-0">
                                                <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-slate-300 group-hover:text-violet-600 shadow-sm transition-colors border border-slate-100 flex-shrink-0">
                                                    <ReceiptText size={18} />
                                                </div>
                        ...
                        <div className="px-8 pt-8 flex items-center justify-between mb-8">
                            <div className="flex flex-wrap items-center gap-3">
                                <button
                                    ...
                                    className={`flex items-center gap-2.5 px-6 py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all ${activeTab === 'income' ? 'bg-violet-600 text-white shadow-lg shadow-violet-200' : 'bg-slate-50 text-slate-400 hover:text-slate-600'}`}
                                >
                                            ...
                                            transactions.filter(t => t.flow === (activeTab === 'income' ? 'in' : 'out')).map((txn, idx) => (
                                                <tr key={idx} className="group hover:bg-slate-50/50 transition-all">
                                                    ...
                                                    <td className="py-5 pr-4 text-center">
                                                        <span className="text-[10px] font-black text-slate-400 group-hover:text-violet-600 underline cursor-pointer uppercase tracking-widest">{txn.id}</span>
                                                    </td>
                                                    <td className="py-5 pr-4 text-center">
